...
| ブロック | 内容 | 反映のタイミング |
|---|---|---|
| **抽出条件** | プリセット / 含む / 除外 / パターンの解釈 / 大文字小文字 | `適用`（または「自動適用」）を押したとき |
...
- 保存される内容: 対象ファイル / 文字コード / positive・negative / 検索方法 / 大文字小文字 /
  AND・OR / 前後 n 行 / マーカー / 折り返し・行番号・強調・フォント / 検索語 / カーソル行
...
- マーカー行は、左端のオレンジの帯・淡いオレンジの行背景・オレンジ太字の行番号の
  3 つで示される（横スクロールで帯が画面外へ出ても背景色で分かる）
